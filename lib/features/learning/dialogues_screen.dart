...
class _DialoguesScreenState extends State<DialoguesScreen> with SingleTickerProviderStateMixin, HelpFeatureMixin {
              ...
              children: [
                if (memo.type == MemoType.idea)
                  IconButton(
                    ...
                    onPressed: () => _convertToHabit(memo), 
                    tooltip: 'تحويل لعادة',
                  ),
                if (memo.type == MemoType.idea || (memo.type == MemoType.data && memo.dataCategory == 'قرارات'))
                  IconButton(
                    icon: const Icon(Icons.assignment_turned_in_outlined, color: Colors.blue), 
                    onPressed: () => _convertToTask(memo), 
                    tooltip: 'تحويل لمهمة',
                  ),
  ...
  void _convertToTask(Memo memo) {
    ...
    ModernDialog.show(
      context: context,
      title: 'تحويل إلى مهمة سريعة',
      content: StatefulBuilder(
        builder: (context, setModalState) => Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text('المحتوى: ${memo.content}', style: const TextStyle(fontSize: 12, color: Colors.grey)),
            ...
            DropdownButtonFormField<TaskPriority>(
              ...
              items: TaskPriority.values.map((p) => DropdownMenuItem(
                value: p, 
                child: Text(p == TaskPriority.urgent ? 'عاجل 🚩' : (p == TaskPriority.medium ? 'متوسط ⚠️' : 'عادي ✅'))
              )).toList(),
        ...
        ElevatedButton(
          onPressed: () async {
            final task = Task(
              ...
              title: memo.content.length > 30 ? memo.content.substring(0, 27) + '...' : memo.content,
