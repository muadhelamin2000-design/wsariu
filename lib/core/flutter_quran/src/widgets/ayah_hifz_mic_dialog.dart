...
class AyahHifzMicDialog extends StatefulWidget {
  final Ayah ayah;
  const AyahHifzMicDialog({required this.ayah, super.key});

  @override
  State<AyahHifzMicDialog> createState() => _AyahHifzMicDialogState();
}

class _AyahHifzMicDialogState extends State<AyahHifzMicDialog> {
  ...
  void initState() {
    ...
    // تنظيف النص من التشكيل وعلامات الوقف للمقارنة الدقيقة
    _ayahWords = widget.ayah.ayah.trim().split(RegExp(r'\s+'));
    _revealed = List.filled(_ayahWords.length, false);
  }
  ...
  void _compareResult(String recognized) {
    ...
    for (var recWord in recognizedWords) {
      ...
      for (int i = 0; i < _ayahWords.length; i++) {
        if (!_revealed[i]) {
          String normAyah = ArabicText.normalize(_ayahWords[i]);
          // مقارنة الكلمات مع تجاهل التشكيل
  ...
  bool _isSimilar(String s1, String s2) {
    // منطق بسيط للمقارنة في حال كانت الكلمة تحتوي على حروف متشابهة في النطق أو أخطاء بسيطة
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 8.h),
            Text(
              "سورة ${widget.ayah.surahNameAr} - آية ${widget.ayah.ayahNumber}",
              style: TextStyle(fontSize: 12.sp, color: Colors.grey, fontFamily: 'Cairo'),
            ),
            ...
            Container(
              ...
              child: SingleChildScrollView(
                child: Wrap(
                  ...
                  spacing: 6,
                  runSpacing: 10,
                  children: List.generate(_ayahWords.length, (index) {
            ...
            TextButton(
              ...
              child: const Text('إغلاق الاختبار', style: TextStyle(fontFamily: 'Cairo')),
            ),
          ],
        ),
      ),
